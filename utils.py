...
def add_label_in_db(label_name, label_color):
    conn = sqlite3.connect('todo.db')
    cursor = conn.cursor()
    cursor.execute("INSERT INTO label (name, color) VALUES (?, ?)", (label_name, label_color))
    last_label_id = cursor.lastrowid
    conn.commit()
    conn.close()
...
def add_task_to_api(task):
    ...
    response = requests.post("http://localhost:3000/api/tasks", json=payload)
    if response.status_code == 200:
        print("タスクが追加されました。")
    else:
        print("タスクの追加に失敗しました。")
...
def update_task_to_api(task):
    ...
    response = requests.patch(f"http://localhost:3000/api/tasks/{task_id}", json=payload)
    if response.status_code == 200:
        print("タスクが追加されました。")
    else:
        print("タスクの追加に失敗しました。")
...
def delete_task_to_api(task_id):
    response = requests.delete(f"http://localhost:3000/api/tasks/{task_id}")
    if response.status_code == 200:
        print("タスクが追加されました。")
    else:
        print("タスクの追加に失敗しました。")
